...
const locationToAddress = async (location) => {
    let data = undefined
    await axios.get(`https://maps.googleapis.com/maps/api/geocode/json?latlng=${location.lat},${location.lng}&key=AIzaSyAvSIFGIo-hmpQwRS-SKcUkqAepbT3LzVA`)
        .then((res) => {
...
io.on("connection", socket => {
    ...
    socket.on('customerLocation', async (data) => {
        ...
        if (isAutoPilot) {
            ...
            let customer = await instuserLocation.findOne({
                status: 'online',
            }).sort({
                createdAt: -1
            }).limit(1)
            ...
            if (drivers.length > 0 && customer)
                await axios.post('https://www.turkpark.com.tr:2222/calcLoc/auto', {
                    user: {
    ...
    socket.on('driverLocation', async (data) => {
        ...
        // sort created at
        let driverLog = await driverLocationData.find({
            driverID: driver.id
        }).sort({
            timestamp: -1
        });
        ...
        let x = await trackDriverDB.findOneAndUpdate(filter, {
            lat: driver.lat,
            status: driver.status,
            lng: driver.lng,
    ...
    socket.on('logout', (data) => {
        ...
        trackDriverDB.findOneAndUpdate(filter, {
            status: "logout"
        }, {
            new: true
        }, (err, result) => {
    ...
    socket.on('endTrip', async (data) => {
        ...
        let allTripsData = await allTripsDB.find({
            trip_id: data.tripId
        }).sort({
            created_at: -1
        })
        ...
        let partnerPrice = user.user_type === 'partner' ? finalData.price - (finalData.price * 0.10) : 0
