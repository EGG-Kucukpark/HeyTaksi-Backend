...
const {
  db,
  findById
} = require('../../models/operator/user/userDB');
...
router.get('/heytaksi/log', async (req, res) => {
  res.json(await heytaksi_log.find().sort({
    created_at: -1
  }))
})
...
router.put('/options', async (req, res) => {
  ...
  generalDB.findByIdAndUpdate('6232f38425c35446ed173925', {
    price,
    waitPrice,
    minPrice,
    driverDiscount,
...
router.delete('/timeOptions', async (req, res) => {
  ...
  await db.prices[day].splice(db.prices[day].findIndex(x => x.id == id), 1)
