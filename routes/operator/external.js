...
router.post('/calcPrice', async (req, res) => {
    ...
    await axios(encodeURI(`https://maps.googleapis.com/maps/api/geocode/json?address=${begin}&key=AIzaSyAvSIFGIo-hmpQwRS-SKcUkqAepbT3LzVA`)).then((res) => {
        loc1 = res.data.results[0].geometry.location;
    }).catch((err) => {
        console.log(err)
    })

    await axios(encodeURI(`https://maps.googleapis.com/maps/api/geocode/json?address=${end}&key=AIzaSyAvSIFGIo-hmpQwRS-SKcUkqAepbT3LzVA`)).then((res) => {
    ...
    if (loc1 != null && loc2 != null) {
        await axios(encodeURI(`https://maps.googleapis.com/maps/api/distancematrix/json?units=imperial&origins=${loc1.lat},${loc1.lng}&destinations=${loc2.lat},${loc2.lng}&key=AIzaSyAvSIFGIo-hmpQwRS-SKcUkqAepbT3LzVA`)).then((x) => {
...
router.post('/textloc', async (req, res) => {
    ...
    await axios(encodeURI(`https://maps.googleapis.com/maps/api/geocode/json?address=${req.body.address_text}+Izmir&key=AIzaSyAvSIFGIo-hmpQwRS-SKcUkqAepbT3LzVA`)).then((res) => {
...
router.post('/getLoc', async (req, res) => {
    ...
    let data = undefined
    await axios.get(`https://maps.googleapis.com/maps/api/geocode/json?latlng=${req.body.loc.lat},${req.body.loc.lng}&key=AIzaSyAvSIFGIo-hmpQwRS-SKcUkqAepbT3LzVA`)
        .then((res) => {
