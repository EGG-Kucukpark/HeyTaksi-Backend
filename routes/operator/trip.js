...
router.get('/currentTripData', async (req, res) => {
    ...
    let allTrips = await allTrips.find(filter).sort({
        created_at: -1
    }).limit(1);
...
router.post('/trip', async (req, res) => {
    ...
    let db = (await instuserLocation.find({
...
router.post('/interCity', async (req, res) => {
    ...
    await interCity.findByIdAndUpdate(data._id, {
        $push: {
            users: req.body.user
        }
    }).then((res) => {
...
router.put('/interCity', async (req, res) => {
    ...
    x.users.map(async (user) => {
        ...
        if (user.phone === userData.phone) {
            ...
            await interCity.findByIdAndUpdate(req.body.trip_id, {
                $push: {
                    users: userData
                }
            }).then((res) => {
